...
#    as part of convective_transition_diag_v1r0.py
#
#   Including:
...
def convecTransCriticalCollapse_plot(argsv1,argsv2,argsv3,argsv4,argsv5,argsv6):
    ...
    if (p1_obs!=[]):
        ...
        for reg in numpy.arange(NUMBER_OF_REGIONS):
            ...
            ax2.text(0.05, 0.7, REGION_STR[reg], transform=ax2.transAxes, fontsize=12, fontweight="bold", verticalalignment="top")
            ax2.grid()
        ...
        fig_obs_cts.savefig(FIG_OBS_DIR+"/"+FIG_OBS_FILENAME_CTS, bbox_inches="tight", bbox_extra_artists=(leg,))

        ##### Figure Critical CWV (WC) #####
        fig_obs_wc = mp.figure(figsize=(figsize1/1.8,figsize2/4))
        ...
        leg = ax1.legend(handles, labels, fontsize=axes_fontsize, bbox_to_anchor=(0.05,0.95), \
                        ...
                        fancybox=False,scatterpoints=1,  framealpha=0, borderpad=0, \
                        handletextpad=0.1, markerscale=1, ncol=1, columnspacing=0.25)
        ax1.text(0.5, 0.2, OBS, transform=ax1.transAxes, fontsize=12, fontweight="bold", verticalalignment="top")
        ax1.text(0.5, 0.1, RES+"$^{\circ}$", transform=ax1.transAxes, fontsize=12, fontweight="bold", verticalalignment="top")
        ax1.set_xlabel(TEMP_VAR+" "+TEMP_UNITS, fontsize=axes_fontsize)
        ax1.set_ylabel("$w_c$ (mm)", fontsize=axes_fontsize)
        ax1.grid()
        ax1.set_axisbelow(True)
        ...
        leg = ax2.legend(handles, labels, fontsize=axes_fontsize, bbox_to_anchor=(0.6,0.95), \
                        ...
                        fancybox=False,scatterpoints=1,  framealpha=0, borderpad=0, \
                        handletextpad=0.1, markerscale=1, ncol=1, columnspacing=0.25)
        ax2.text(0.2, 0.2, OBS, transform=ax2.transAxes, fontsize=12, fontweight="bold", verticalalignment="top")
        ax2.text(0.2, 0.1, RES+"$^{\circ}$", transform=ax2.transAxes, fontsize=12, fontweight="bold", verticalalignment="top")
        ax2.set_xlabel(TEMP_VAR+" "+TEMP_UNITS, fontsize=axes_fontsize)
        ax2.set_ylabel("$w_c/qsat_{int}$", fontsize=axes_fontsize)
        ax2.grid()
        ax2.set_axisbelow(True)

        # set layout to tight (so that space between figures is minimized)
        fig_obs_wc.tight_layout()
        fig_obs_wc.savefig(FIG_OBS_DIR+"/"+FIG_OBS_FILENAME_WC, bbox_inches="tight", bbox_extra_artists=(leg,))
    ...
    for reg in numpy.arange(NUMBER_OF_REGIONS):
        ...
        ax2.text(0.05, 0.8, REGION_STR[reg], transform=ax2.transAxes, fontsize=12, fontweight="bold", verticalalignment="top")
        ax2.grid()
        ax2.grid(b=True, which='minor', color='0.8', linestyle='-')
        ax2.set_axisbelow(True)
    ...
    fig_cts.savefig(FIG_OUTPUT_DIR+"/"+FIG_FILENAME_CTS, bbox_inches="tight", bbox_extra_artists=(leg,))

    ##### Figure Critical CWV (WC) #####
    fig_wc = mp.figure(figsize=(figsize1/1.8,figsize2/3.))
    ...
    if (OVERLAY_OBS_ON_TOP_OF_MODEL_FIG and p1_obs!=[]):
        ax1.text(0.6, 0.3, OBS, transform=ax1.transAxes, fontsize=12, fontweight="bold", verticalalignment="top",color='0.6')
        ax1.text(0.6, 0.2, RES+"$^{\circ}$", transform=ax1.transAxes, fontsize=12, fontweight="bold", verticalalignment="top",color='0.6')
    ax1.set_xlabel(TEMP_VAR+" "+TEMP_UNITS, fontsize=axes_fontsize)
    ax1.set_ylabel("$w_c$ (mm)", fontsize=axes_fontsize)
    ax1.grid()
    ax1.set_axisbelow(True)
    ...
    if (OVERLAY_OBS_ON_TOP_OF_MODEL_FIG and p1_obs!=[]):
        ax2.text(0.18, 0.27, OBS, transform=ax2.transAxes, fontsize=12, fontweight="bold", verticalalignment="top",color='0.6')
        ax2.text(0.18, 0.17, RES+"$^{\circ}$", transform=ax2.transAxes, fontsize=12, fontweight="bold", verticalalignment="top",color='0.6')
    ax2.set_xlabel(TEMP_VAR+" "+TEMP_UNITS, fontsize=axes_fontsize)
    ax2.set_ylabel("$w_c$/qsat_int", fontsize=axes_fontsize)
    ax2.grid()
    ax2.set_axisbelow(True)

    # set layout to tight (so that space between figures is minimized)
    fig_wc.tight_layout()
    fig_wc.savefig(FIG_OUTPUT_DIR+"/"+FIG_FILENAME_WC, bbox_inches="tight", bbox_extra_artists=(leg,))
